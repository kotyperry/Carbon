        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-xl sm:text-2xl font-mono font-bold truncate flex items-center gap-2">
            {activeTag && currentTagData ? (
              <>
                <span className={`w-3 h-3 rounded-full ${currentTagData.color}`} />
                {currentTagData.name}
              </>
            ) : (
      ...
      <div className="flex-1 overflow-auto p-4 sm:p-6">
        ...
        {allTags.length > 0 && (
          <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-2">
            <span className={`text-xs font-mono uppercase tracking-wider shrink-0 ${theme === 'dark' ? 'text-gray-500' : 'text-gray-400'}`}>Tags:</span>
            {allTags.map(tagKey => {
        ...
        {bookmarks.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {bookmarks.map((bookmark) => (
              <BookmarkCard key={bookmark.id} bookmark={bookmark} onEdit={handleEditBookmark} />
            ))}
